...
@section('title', 'Create New Specialization')
        ...
        <form action="{{ route('admin.specializations.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" required class="w-full px-4 py-2.5 text-gray-900 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-150 ease-in-out">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
